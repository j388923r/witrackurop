...
class AddDeviceSegue: UIStoryboardSegue {
    override func perform() {
        // Assign the source and destination views to local variables.
        let firstVC = self.sourceViewController as! DeviceSelectorViewController
        let navVC = self.destinationViewController as! UINavigationController
        let secondVC = navVC.topViewController as! AddDeviceTableViewController
        
        secondVC.userId = firstVC.userId!
        secondVC.token = firstVC.token!
        secondVC.devices = firstVC.devices
